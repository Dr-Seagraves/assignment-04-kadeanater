...
def plot_scatter_with_regression(
    ...
    df_plot = df.dropna(subset=[x_var, 'ret'])
    ax.scatter(df_plot[x_var], df_plot['ret'], alpha=0.5, label='Data points')
    # Regression line
    ...
    x_lower = df_plot[x_var].quantile(0.02)
    x_upper = df_plot[x_var].quantile(0.98)
    y_lower = df_plot['ret'].quantile(0.02)
    y_upper = df_plot['ret'].quantile(0.98)
...
def print_key_results(model, x_var: str) -> None:
    ...
    print(f"Intercept (β₀): {model.params['Intercept']:.4f} (SE: {model.bse['Intercept']:.4f}, t: {model.tvalues['Intercept']:.4f}, p: {model.pvalues['Intercept']:.4f})")
    print(f"Slope (β₁): {model.params[x_var]:.4f} (SE: {model.bse[x_var]:.4f}, t: {model.tvalues[x_var]:.4f}, p: {model.pvalues[x_var]:.4f})")
    print(f"R²: {model.rsquared:.4f}, Adj R²: {model.rsquared_adj:.4f}, N: {model.nobs:.0f}")
    if model.pvalues[x_var] < 0.05:
        sign = "positive" if model.params[x_var] > 0 else "negative"
        print(f"The slope is {sign} and statistically significant at 5%.")
    else:
        print("The slope is not statistically significant at 5%.")
    print("=" * 60 + "\n")
